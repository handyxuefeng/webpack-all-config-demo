 ...
 * 1. 自动向产出的html里面插入script标签，标签src 指向库的cdn 的url地址
 * 2. 当引入 lodash， jquery模块的时候，自动转为外部模块，不再打包次模块到chunk上，读的是window 上 jquery 和 _ 变量
 * opts = {
        ...
        */
        ...
        compiler.hooks.compilation.tap("AutoExternalPlugin", (compilation) => {
            ...
            HtmlWebpackPlugin.getHooks(compilation).alterAssetTags.tapAsync('AutoExternalPlugin', (HtmlWebpackPluginData, callback) => {
                ...
                let optsArray = Object.values(this.opts);
                let insertScripts = optsArray.map(item => {
                    return item.url;
                });
